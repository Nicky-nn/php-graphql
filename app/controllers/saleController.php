...
class saleController extends mainModel
{
	...
	// Contralodor listar facturas
	public function listarFacturaControlados($pagina, $registros, $url, $busqueda)
	{
		$pagina = $this->limpiarCadena($pagina);
		$registros = $this->limpiarCadena($registros);
		$url = $this->limpiarCadena($url);
		...
		$datos_api = $this->obtenerFacturasAPI($pagina, $registros);

		// Ordenar por fecha de emisión
		usort($datos_api['docs'], function ($a, $b) {
			return strtotime($b['fechaEmision']) - strtotime($a['fechaEmision']);
		});

		$total = $datos_api['pageInfo']['totalDocs'];
		$numeroPaginas = ceil($total / $registros);

		$tabla .= $this->generarTablaHTML($datos_api['docs']);
	...
	private function obtenerFacturasAPI($pagina, $limite)
	{
		$url = "https://sandbox.isipass.net/api";
		$token = $_SESSION['isipass_token'] ?? '';
		...
		$query = '
    {
		...
		return $data['data']['facturaCompraVentaAll'] ?? ['docs' => [], 'pageInfo' => ['totalDocs' => 0]];
	}

	private function generarTablaHTML($datos)
	{
		$tabla = '
    <div class="table-container">
    <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
        <thead>
            <tr>
                <th class="has-text-centered">CUF.</th>
                <th class="has-text-centered">Nro. Factura</th>
                <th class="has-text-centered">Fecha</th>
                <th class="has-text-centered">Cliente</th>
                <th class="has-text-centered">Vendedor</th>
                <th class="has-text-centered">Total</th>
		...
		if (count($datos) >= 1) {
			foreach ($datos as $rows) {
				$pdfUrl = htmlspecialchars($rows['representacionGrafica']['pdf'], ENT_QUOTES, 'UTF-8');
				$rolloUrl = htmlspecialchars($rows['representacionGrafica']['rollo'], ENT_QUOTES, 'UTF-8');
				$sin = htmlspecialchars($rows['representacionGrafica']['sin'], ENT_QUOTES, 'UTF-8');
				$tabla .= '
                <tr class="has-text-centered">
                    <td>' . $rows['_id'] . '</td>
                    <td>' . $rows['numeroFactura'] . '</td>
                    <td>' . $rows['fechaEmision'] . '</td>
                    <td>' . $this->limitarCadena($rows['cliente']['razonSocial'], 30, "...") . '</td>
                    <td>' . $this->limitarCadena($rows['usucre'], 30, "...") . '</td>
                    <td>' . MONEDA_SIMBOLO . number_format($rows['montoTotal'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR) . ' ' . MONEDA_NOMBRE . '</td>
					<td class="has-text-centered" style="color: ' . ($rows['state'] == 'VALIDADA' ? 'green' : 'red') . ';">' . $rows['state'] . '</td>
                    <td>
                    <button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="window.open(\'' . $pdfUrl . '\', \'_blank\')" title="Imprimir factura API Nro. ' . htmlspecialchars($rows['_id'], ENT_QUOTES, 'UTF-8') . '">
                        <i class="fas fa-file-invoice-dollar fa-fw"></i>
                    </button>
                    <button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="window.open(\'' . $rolloUrl . '\', \'_blank\')" title="Imprimir ticket API Nro. ' . htmlspecialchars($rows['_id'], ENT_QUOTES, 'UTF-8') . '">
                        <i class="fas fa-receipt fa-fw"></i>
                    </button>
					<button type="button" class="button is-link is-outlined is-rounded is-small btn-sale-options" onclick="window.open(\'' . $sin . '\', \'_blank\')" title="Imprimir sin API Nro. ' . htmlspecialchars($rows['_id'], ENT_QUOTES, 'UTF-8') . '">
